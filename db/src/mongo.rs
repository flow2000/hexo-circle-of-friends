...
/// 获取`posts`表中最近一次更新（`createdAt`最新）的时�?
...
/// 查询`friends`表中`link`包含`domain_str`的一条数�?
...
/// 查询`posts`表中`link`包含`domain_str`的数�?
///
/// 当num<0时，返回所有数�?
...
mod tests {
    ...
    async fn test_delete_outdated_posts() {
        ...
        assert!(remaining_posts.iter().any(|p| p.meta.title == "新帖�?));
        assert!(remaining_posts.iter().any(|p| p.meta.title == "旧帖�?"));
    }
    ...
    async fn test_insert_and_select_article_summary() {
        ...
        assert_eq!(found_summary.summary, "这是一个测试摘�?);
    }
    ...
    async fn test_select_article_summary_by_hash() {
        ...
        assert_eq!(found_summary.summary, "根据哈希查询的摘�?);
    }
}
