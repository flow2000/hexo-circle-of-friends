...
mod tests {
    ...
    async fn setup_test_db() -> MySqlPool {
        // 从环境变量获取测试数据库连接字符串
        let mysqlconnstr = "mysql://root:123456@localhost:3306/pyq";
        let dbpool = match connect_mysql_dbpool(mysqlconnstr).await {
            Ok(dbpool) => dbpool,
            Err(e) => {
                panic!("{}", e);
            }
    ...
    // 辅助函数：清空表
    async fn truncate_table(pool: &MySqlPool, table: &str) -> Result<(), Error> {
        let sql = format!("TRUNCATE TABLE {table}");
        query(&sql).execute(pool).await?;
        Ok(())
    }

    // 测试连接数据库 - 需要跳过，除非有测试环境
    #[tokio::test]
    async fn test_connect_mysql_dbpool() {
        // 从环境变量获取测试数据库连接字符串
        let mysqlconnstr = "mysql://root:123456@localhost:3306/pyq";
        let dbpool = match connect_mysql_dbpool(mysqlconnstr).await {
            Ok(dbpool) => dbpool,
